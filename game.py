        ...
        for i in range(len(self.agents)):
            ...
            if ("register_initial_state" in dir(agent)):
                ...
                if self.catchExceptions:
                    ...
                    timed_func = TimeoutFunction(agent.register_initial_state, int(math.ceil(self.state.data.score / SCALING_FACTOR)))
                    try:
                        start_time = time.time()
                        timed_func(self.state.deepCopy())
                    except TimeoutFunctionException:
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    ...
                    timed_func = TimeoutFunction(agent.get_action, int(self.state.data.score))
                    try:
                        start_time = time.time()
                        if skip_action:
                    ...
                    #     print("Agent %d took too long to make a move! This is warning %d" % (agent_index, self.totalAgentTimeWarnings[agent_index]), file=sys.stderr)
                    #     if self.totalAgentTimeWarnings[agent_index] > self.rules.getMaxTimeWarnings(agent_index):
                    #         print("Agent %d exceeded the maximum number of warnings: %d" % (agent_index, self.totalAgentTimeWarnings[agent_index]), file=sys.stderr)
                    #         self.agentTimeout = True
                    #         self._agentCrash(agent_index, quiet=True)
                    #         self.unmute()
                    #         return
                    ...
                    #     print("Agent %d ran out of time! (time: %1.2f)" % (agent_index, self.totalAgentTimes[agent_index]), file=sys.stderr)
                    #     self.agentTimeout = True
                    #     self._agentCrash(agent_index, quiet=True)
                    #     self.unmute()
                    #     return
                ...
                try:
                    timed_func = TimeoutFunction(agent.get_action, int(math.ceil(self.state.data.score / SCALING_FACTOR)))
                    try:
                        start_time = time.time()
                        action = timed_func(observation)
                    except TimeoutFunctionException:
