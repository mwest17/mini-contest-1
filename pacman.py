...
#        GameState.explored.add(self)
        ...
        if agent_index < self.data.numPacmanAgents:
            return PacmanRules.getLegalActions(self, agent_index)
        else:
    ...
    def generateSuccessor( self, agent_index, action):
        ...
        if agent_index < self.data.numPacmanAgents:  # Pacman is moving
            # TODO look into how eaten works - currenlty not current for ghosts
            state.data._eaten = [False for i in range(state.getNumAgents())]
            PacmanRules.applyAction( state, action, agent_index )
        ...
        # GhostRules.checkDeath( state, agent_index, self.data.numPacmanAgents, self.data.numGhostAgents )
    ...
    def getLegalPacmanActions( self, agent_index ):
        return self.getLegalActions( agent_index )

    def generatePacmanSuccessor( self, action, agent_index ):
        """
        Generates the successor state after the specified pacman move
        """
        return self.generateSuccessor( agent_index, action )

    def getPacmanState( self, agent_index ):
        """
        ...
        """
        return self.data.agentStates[agent_index].copy()

    def getPacmanStates( self ):
        return self.data.agentStates[:self.data.numPacmanAgents]

    def get_pacman_position( self, agent_index ):
        return self.data.agentStates[agent_index].getPosition()

    def get_pacman_positions(self):
        return [self.get_pacman_position(i) for i in range(self.data.numPacmanAgents)]
...
class PacmanRules:
    ...
    def getLegalActions( state, agent_index ):
        """
        Returns a list of possible actions.
        """
        return Actions.getPossibleActions( state.getPacmanState(agent_index).configuration, state.data.layout.walls )
    getLegalActions = staticmethod( getLegalActions )

    def applyAction( state, action, agent_index ):
        """
        Edits the state to reflect the results of the action.
        """
        legal = PacmanRules.getLegalActions( state, agent_index )
        if action not in legal:
            raise Exception("Illegal action " + str(action))

        pacmanState = state.data.agentStates[agent_index]
...
class GhostRules:
    ...
    def checkDeath(state, agent_index, numPacmanAgents, numGhostAgents):
        pacmanPositions = state.get_pacman_positions()
        if agent_index < numPacmanAgents: # Pacman just moved; Anyone can kill him
    ...
    def collide( state, ghostState, agent_index):
...
def loadPacmanAgent(pacman, nographics):
    ...
    module = __import__(module_name[:-3], fromlist=('create_agents'))
    return module.create_agents
